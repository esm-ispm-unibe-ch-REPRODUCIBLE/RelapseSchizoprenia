---
title: "Network Meta-analysis for the secondary outcome of side effects: Change in Body weight"
...
I excluded all the studies that did not have the informations about Weight_participants, Weight_mean or Weight_sd
```{r, echo=FALSE, message=FALSE, warning=FALSE}
DATA<-DATA1[!is.na(DATA1$Weight_participants),]
DATA<-DATA[!is.na(DATA$Weight_mean),]
DATA<-DATA[!is.na(DATA$Weight_sd),]

excludeID <-c(4330,4334,4360,4373,4380,4416)
DATA=DATA[!DATA$Final_ID_all%in%excludeID,]
```

#Description of the network

Below is a description for the network formed by studies examining the secondary outcome of side effects Change in Body Weight.

Below are the total number of participants in each of the included drug
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#describe the data
tapply(DATA$Weight_participants,DATA$Drug_name,sum,na.rm=T)
...
The network consists of 25 two-arm studies and 1 three-arm study.

The plot below shows the available data

```{r, include=FALSE}
#prepare the data
DATApairs=pairwise(treat=Drug_name, mean=Weight_mean, sd=Weight_sd, n=Weight_participants, data=DATA, sm="MD", studlab=Final_ID_all, allstudies = TRUE)

#run NMA and create an object called EFF for efficacy
#run NMA and create an object called EFF for efficacy
net1<-netmeta(TE, seTE, treat1, treat2, studlab, data = DATApairs, sm = "MD", comb.random=TRUE, comb.fixed=FALSE, prediction=TRUE, ref="Placebo")
...
#forest plots
forest(net1, ref="Placebo", sortvar = -Pscore,xlab="MD")

forest(net1, ref="Placebo", xlab="MD")
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
NMAdataContinuous=make.jagsNMA.data(studyid=Final_ID_all,t=Drug_name,y=Weight_mean, sd=Weight_sd, n=Weight_participants,data=DATA,type="cont",reference = "Placebo")
...
#Comparison between frequentist MDs and Bayesian MDs 

```{r, include=FALSE}
MDFrequentist<-(net1$TE.random[,dimnames="Placebo"])
