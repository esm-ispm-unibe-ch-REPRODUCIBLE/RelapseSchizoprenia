...
I excluded all the studies that did not have the informations about Prolactin_participants, Prolactin_mean or Prolactin_sd and also 4468 study because of disconnected network
```{r, echo=FALSE, message=FALSE, warning=FALSE}
DATA<-DATA1[!is.na(DATA1$Prolactin_participants),]
DATA<-DATA[!is.na(DATA$Prolactin_mean),]
DATA<-DATA[!is.na(DATA$Prolactin_sd),]
excludeID <-c(4330,4334,4360,4373,4380,4416,4496,4468)
...
The network consists of 19 two-arm studies and 1 three-arm study.
...
The p-value is lower than 0.10 and the inconsistency of the comparisons are really high (35.7% > 10%). NMA is not recommended for this network.
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
NMAdataContinuous=make.jagsNMA.data(studyid=Final_ID_all,t=Drug_name,y=Prolactin_mean, sd=Prolactin_sd, n=Prolactin_participants,data=DATA,type="cont",reference = "Placebo")
