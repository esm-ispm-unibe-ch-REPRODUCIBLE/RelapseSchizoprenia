---
title: "Network Meta-analysis for the secondary outcome of side effects: Change in QTC"
...
I excluded all the studies that did not have the informations about QTc_participants, QTc_mean or QTc_sd and also studies 
```{r, echo=FALSE, message=FALSE, warning=FALSE}
DATA<-DATA1[!is.na(DATA1$QTc_participants),]
DATA<-DATA[!is.na(DATA$QTc_mean),]
DATA<-DATA[!is.na(DATA$QTc_sd),]
excludeID <-c(4334,4380)
...
Below is a description for the network formed by studies examining the secondary outcome of side effects Change in QTc.

Below are the total number of participants in each of the included drug
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#describe the data
tapply(DATA$QTc_participants,DATA$Drug_name,sum,na.rm=T)
...
The network consists of 11 two-arm studies.

The plot below shows the available data

```{r, include=FALSE}
#prepare the data
DATApairs=pairwise(treat=Drug_name, mean=QTc_mean, sd=QTc_sd, n=QTc_participants, data=DATA, sm="MD", studlab=Final_ID_all, allstudies = TRUE)
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
NMAdataContinuous=make.jagsNMA.data(studyid=Final_ID_all,t=Drug_name,y=QTc_mean, sd=QTc_sd, n=QTc_participants,data=DATA,type="cont",reference = "Placebo")
