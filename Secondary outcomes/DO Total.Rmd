...
The network consists of 86 two-arm studies and 4 three-arm studies.
...
#forest plots
forest(net1, ref="Placebo oral or depot", sortvar = -Pscore,xlab="OR",xlim=c(0.01,5))

forest(net1, ref="Placebo oral or depot", xlab="OR",xlim=c(0.01,5))
```
...
Because p-value < 0.10 and the incosistent comparisons > 10%, it is not recommended to perform NMA.
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
NMAdataBinary=make.jagsNMA.data(studyid=Final_ID_all,t=Drug_name,r=N_DO_AnyReason,n=N_randomized,data=DATA,type="binary",reference = "Placebo oral or depot")
...
ORFrequentist<-as.data.frame(ORFrequentist[-20,])
